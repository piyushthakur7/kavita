...
export default function EditBlogPage({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || !excerpt || !content || !coverImage) {
      setError('Please populate all required fields, including cover image and body content.');
  ...
  if (fetching) {
    return (
      <main className="min-h-screen bg-[#070608] text-[#e5dcd3] flex items-center justify-center relative">
        <div className="absolute inset-0 bg-speaker-mesh opacity-20 pointer-events-none" />
        <div className="text-center relative z-10">
          <RefreshCw size={36} className="text-[#c5a880] animate-spin mx-auto mb-4" />
      ...
      <header className="border-b border-[#c5a880]/15 bg-[#0c0a0e]/95 backdrop-blur sticky top-0 z-30 px-6 lg:px-10 py-4 shadow-xl">
        <div className="max-w-[1200px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-serif-display text-xl text-white">RE-TUNING CHANNEL BOARD</h1>
          </div>
          <Logo light />
        </div>
      </header>
        ...
        <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-1 space-y-6 sticky top-24">
            ...
            <div className="bg-[#0c0a0e] border border-white/5 p-6 rounded-2xl shadow-inner">
              ...
              >
                <Save size={14} />
                <span>{loading ? 'MODULATING CHANGES...' : 'SAVE MODULATIONS'}</span>
              </button>
